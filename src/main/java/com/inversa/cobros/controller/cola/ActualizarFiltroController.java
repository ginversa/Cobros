...
public class ActualizarFiltroController implements Serializable {
    ...
    public void generarCola() {
        String usuario = this.usuario.getUsuarioingreso();
        Date fecha = this.fechaHoy.getTime();
        try {
            if (this.filtrocola != null && this.filtrocola.getIdFiltrocola() != null) {
                this.ejbFiltrocolaLocal.deleteCola(filtrocola);
                this.ejbFiltrocolaLocal.generarCola(this.filtrocola,usuario,fecha);
                this.filtrocola = this.ejbFiltrocolaLocal.findByIdFiltrocola(this.filtrocola);
                PrimeFaces.current().ajax().update("formCola");
            ...
            Logger.getLogger(ActualizarFiltroController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
